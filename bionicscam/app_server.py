...
try:
    from .integrations.google_drive import upload_gcode_to_drive, GoogleDriveUploader
...
try:
    from .integrations.penguin_auth import init_auth
    AUTH_AVAILABLE = True
except ImportError:
...
try:
    from .integrations.onshape import get_onshape_client, session_manager
...
from .postprocessor import FRCPostProcessor, PostProcessorResult
...
app = Flask(__name__, template_folder=str(WEB_DIR / 'templates'), static_folder=str(WEB_DIR / 'static'), static_url_path='/static')
...
POST_PROCESSOR = PROJECT_ROOT / 'bionicscam' / 'postprocessor.py'
